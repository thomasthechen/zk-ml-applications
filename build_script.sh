#!/bin/bash
PHASE1=$(pwd)/phase1/powersOfTau28_hez_final_19.ptau
CIRCUIT_NAME=$1
CIRCUITS=$(pwd)/circuits
...
echo "building"
echo "$CIRCUITS/$CIRCUIT_NAME".circom
start=`date +%s`
circom "$CIRCUITS/$CIRCUIT_NAME".circom   --O1 --r1cs --wasm --c --sym --output "$BUILD_DIR"

end=`date +%s`

# THIS DOES NOT SEEM TO WORK ON MACs, SEEMS WORTH LOOKING INTO IF THINGS ARE SLOW
echo "Compiled circuit ($((end-start))s)"
...
echo "DONE Compiling C++ ($((end-start))s)"
...
./"$CIRCUIT_NAME" $CIRCUITS/"$CIRCUIT_NAME".json ../witness.wtns
node generate_witness.js "$CIRCUIT_NAME".wasm $CIRCUITS/"$CIRCUIT_NAME".json ../witness.wtns
end=`date +%s`
